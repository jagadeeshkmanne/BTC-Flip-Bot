...
def main():
    ...
    if exch_pos and pos is None:
        ...
        # Reason heuristic: SL fires when exit price lands ON the SL level.
        # Use distance-from-sl, not side-relative: with BE armed, SL flips to
        # the profitable side of entry (e.g., short SL at firstEntry × 0.9975)
        # which can sit close to prev_mid TP. The old side-based check
        # ("exit_avg >= sl_px * 0.99" for shorts) misclassified those TP fills
        # as SL. 0.5% tolerance covers stop-market slippage.
        sl_px_was = pos.get("sl", 0)
        sl_dist = abs(exit_avg - sl_px_was) / sl_px_was if sl_px_was else 1.0
        reason = "SL" if sl_dist < 0.005 else "TP"
