...
import core_v2_3 as _core_v2_3
_core_v2_3.SL_FROM_WORST = SL_FROM_WORST
...
def main():
    ...
    df_1h["rsi"] = rsi_series(df_1h["close"], RSI_PERIOD)
    # 2026-06-06: ATR(14) for chop-regime filter + 1h cumulative price move
    _prev_close = df_1h["close"].shift(1)
    _tr = pd.concat([
        df_1h["high"] - df_1h["low"],
        (df_1h["high"] - _prev_close).abs(),
        (df_1h["low"]  - _prev_close).abs(),
    ], axis=1).max(axis=1)
    df_1h["atr_14"] = _tr.rolling(14).mean()
    last_idx = len(df_1h) - 2  # last CLOSED 5m bar
    last = df_1h.iloc[last_idx]
    close_px = float(last["close"])
    rsi_val = float(last["rsi"]) if pd.notna(last["rsi"]) else None

    # ── 1h trend gate (entry only) ──
            ...
            if USE_BE_AFTER_DCA and pos.get("filled", 1) >= 2 and BE_WAIT_BARS > 0:
                ...
                if l2_time_str:
                    try:
                        ...
                        bars_since_l2 = int((datetime.now(timezone.utc) - l2_dt).total_seconds() // 3600)
        ...
        if exit_px is None and TIME_SL_BARS > 0:
            ...
            if entry_time_str:
                try:
                    ...
                    bars_held = int((datetime.now(timezone.utc) - entry_dt).total_seconds() // 3600)  # 1h bars
    ...
    if sig and state["position"] is None:
        try:
            atr_val = float(df_1h["atr_14"].iloc[-2])  # ATR at last closed bar
    ...
    if pos:
        ...
        if TIME_SL_BARS > 0 and pos.get("entry_time"):
            try:
                ...
                close_dt = ent_dt + timedelta(minutes=TIME_SL_BARS * 60)
                time_sl_at = close_dt.isoformat()
                bars_elapsed = int((datetime.now(timezone.utc) - ent_dt).total_seconds() // 3600)
    ...
    write_status({
        ...
        "indicators": {"rsi": rsi_val, "rsi_oversold": RSI_OVERSOLD, "rsi_overbought": RSI_OVERBOUGHT,
                       ...
                       "atr_pct": float(df_1h["atr_14"].iloc[-2]) / close_px * 100 if "atr_14" in df_1h.columns and not pd.isna(df_1h["atr_14"].iloc[-2]) else None,
                       "atr_max_pct": RSISCALP_ATR_MAX_PCT,
                       "chg_1h_pct": (close_px / float(df_1h["close"].iloc[-14]) - 1) * 100 if len(df_1h) >= 14 else None,
