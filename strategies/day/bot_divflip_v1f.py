#!/usr/bin/env python3
"""bot_divflip_v1f.py — Paper Bot 4: divflip v1 + improvements.

Identical to bot_divflip.py (v1 paper) EXCEPT three improvements based on
the 15-trade live paper history (2026-05-23):

  1. Removed L3 — DCA_LEVELS=2 (bounds worst-case loss)
  2. SL anchored to first_entry, not worst_entry (catches losses earlier)
  3. 24-hour cooldown after any SL exit (avoids re-entry into downtrend)

Signal, freshness, RSI bands, leverage, TP — all UNCHANGED from v1.

State / log / status: data/paper_divflip_v1f/
"""
...
# ─── Paths ───
DATA_DIR = os.path.join(BOT_DIR, "data", "paper_divflip_v1f")
