...
def main():
    log.info("=" * 60)
    log.info(f"DivFlip Sharp Paper Bot — RSI div {RSI_LONG_MAX:.0f}/{RSI_SHORT_MIN:.0f}, {DCA_LEVELS}-leg DCA, SL {SL_FROM_AVG*100:.1f}%/avg, TP {TP_PCT*100:.1f}%, BE+Trail {BE_TRIGGER_PCT*100:.2f}%/{TRAIL_DIST_PCT*100:.1f}%, UK {UK_HOUR_START}-{UK_HOUR_END} "
    ...
    status = {
        ...
        "stats": state["stats"],
        "strategy": f"DivFlip Sharp [PAPER]: DCA=2 (no L3) / SL {SL_FROM_AVG*100:.2f}% from avg / TP {TP_PCT*100:.1f}% / BE {BE_TRIGGER_PCT*100:.2f}% / Trail {TRAIL_DIST_PCT*100:.1f}% / UK hours UTC {UK_HOUR_START}-{UK_HOUR_END} / no weekday filter / no MAX_LOSS",
        "paper_mode": True,
