...
State / log / status: data/paper_divflip_sharp/
"""
...
from core_divflip_sharp import (
...
# ─── Paths ───
DATA_DIR = os.path.join(BOT_DIR, "data", "paper_divflip_sharp")
...
def main():
    log.info("=" * 60)
    log.info(f"DivFlip Sharp Paper Bot — RSI({RSI_PERIOD}) div {RSI_LONG_MAX:.0f}/{RSI_SHORT_MIN:.0f}, pivot {DIV_PIVOT_L}L/{DIV_PIVOT_R}R, {DCA_LEVELS}-leg DCA @{DCA_SPACING*100:.1f}%, "
             ...
             f"UK {UK_HOUR_START}-{UK_HOUR_END}, {'flip' if USE_FLIP else 'NO flip'}, fresh {DIV_FRESH_BARS}b")
    ...
    if pos:
        ...
        # 24h time-stop on LOSS — force-close if held ≥ TIME_STOP_HOURS AND currently in loss.
        # Saves slow-bleed scenarios (e.g., May 15 v1 trade that bled for 70h to -$701).
        # Winners are NEVER force-closed — they keep running.
            ...
            if entry_time_str:
                ...
                try:
                    ...
                    if hours_held >= TIME_STOP_HOURS and cur_pnl_pct < 0:
                        exit_reason = "TIME24"
                        exit_px = live_px
                        log.warning(f"  TIME24 triggered: held {hours_held:.1f}h ≥ {TIME_STOP_HOURS}h, in loss ({cur_pnl_pct:+.2f}%) — force-closing at ${live_px:.2f}")
    ...
    if state["position"] is None:
        # SHARP entry filters: UK hours + Weekday + same-direction loss cooldown
        ...
        # 6h same-direction cooldown after loss
        cooldown_ok = True
        cooldown_msg = ""
    ...
    status = {
        ...
        "stats": state["stats"],
        "strategy": f"DivFlip Sharp [PAPER]: RSI({RSI_PERIOD}) div {RSI_LONG_MAX:.0f}/{RSI_SHORT_MIN:.0f}, pivot {DIV_PIVOT_L}L/{DIV_PIVOT_R}R, DCA=2 @{DCA_SPACING*100:.1f}% / SL {SL_FROM_AVG*100:.2f}% from avg / TP {TP_PCT*100:.1f}% / BE L1:+{BE_TRIGGER_PCT*100:.2f}% Trail{TRAIL_DIST_PCT*100:.1f}% L2:+{BE_TRIGGER_AFTER_L2*100:.2f}% Trail{TRAIL_DIST_AFTER_L2*100:.1f}% / UK UTC {UK_HOUR_START}-{UK_HOUR_END} / 24h time-stop on loss / {LOSS_COOLDOWN_HOURS}h same-side cooldown after loss",
        "paper_mode": True,
