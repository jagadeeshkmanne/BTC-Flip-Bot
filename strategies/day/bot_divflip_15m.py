#!/usr/bin/env python3
"""bot_divflip_15m.py — Paper Bot 5: divflip v1f on 15-MINUTE timeframe.

15m + retuned params (45-day backtest 2026-04-08 → 2026-05-23):
  Divergence:   7L/1R pivot, RSI period 7, L≤45/S≥65, fresh ≤21 bars
  DCA:          2 levels at 0.75% spacing, weights 3:4
  SL:           2% below FIRST entry (L1-anchored)
  TP:           NONE (let BE+trail handle exits)
  BE:           arms at +2.0% favorable from avg
  Trail:        0.2% buffer below peak
  Cooldown:     NONE (BE 2% bar is the filter)
  Max-hold:     NONE
  Leverage:     3x

Backtest result: +\$3,990 / +79.8% / PF 3.98 / DD 4.7% / 22 trades

State / log / status: data/paper_divflip_15m/
"""
...
# ─── Paths ───
DATA_DIR = os.path.join(BOT_DIR, "data", "paper_divflip_15m")
...
# ─── Logging ───
log = logging.getLogger("bot_divflip_15m")
...
def main():
    ...
    tp_lbl = f"TP {TP_PCT*100:.1f}%" if USE_TAKE_PROFIT else "TP OFF (BE+trail only)"
    cd_lbl = f"{SL_COOLDOWN_HOURS}h cooldown" if SL_COOLDOWN_HOURS > 0 else "no cooldown"
    mh_lbl = f"TIME_STOP {MAX_HOLD_HOURS}h" if MAX_HOLD_HOURS > 0 else "no TIME_STOP"
    log.info(f"Divflip 15m (Paper Bot 5) — RSI{RSI_PERIOD} ≤{RSI_LONG_MAX:.0f}/≥{RSI_SHORT_MIN:.0f} | "
             f"{DIV_PIVOT_L}L/{DIV_PIVOT_R}R pivot | "
             f"{DCA_LEVELS} DCA @ {DCA_SPACING*100:.2f}% mart {':'.join(str(int(r)) for r in [3,4])} | "
             f"SL {SL_FROM_WORST*100:.1f}% {sl_anchor_lbl} | "
             f"{tp_lbl} | BE @{BE_TRIGGER_PCT*100:.1f}% trail {TRAIL_DIST_PCT*100:.2f}% | "
             f"{cd_lbl} | {mh_lbl} | fresh {DIV_FRESH_BARS}b | 15-min bars")

    state = load_state()

    # ─ Fetch market data ─
    df_5m = fetch_klines("15m", 500)   # 15m bars now; variable name kept for code compat
    ...
    if pos:
        ...
        # TIME_STOP: only active if MAX_HOLD_HOURS > 0. 15m config has it OFF.
        if (MAX_HOLD_HOURS > 0 and exit_px is None
                and not pos["be_activated"] and pos.get("entry_time")):
            try:
    ...
    cooldown_msg = ""
    # Cooldown only active if SL_COOLDOWN_HOURS > 0. 15m config has it OFF.
    if SL_COOLDOWN_HOURS > 0:
        ...
        if last_sl:
            ...
            if elapsed < timedelta(hours=SL_COOLDOWN_HOURS):
                ...
                remaining = timedelta(hours=SL_COOLDOWN_HOURS) - elapsed
    ...
    if state["position"] is None:
        ...
        elif new_pos_after_flip:
            if not cooldown_active:
                open_position(state, new_pos_after_flip, live_px, state["balance"])
            else:
                log.info(f"  Flip {new_pos_after_flip} SKIPPED — {cooldown_msg}")
    ...
    tp_str = f"TP {TP_PCT*100:.1f}%" if USE_TAKE_PROFIT else "no TP"
    status = {
        "env": "paper_divflip_15m",
        "timeframe": "15m",
        "pair": PAIR,
        "price": close_px,
        "live_price": live_px,
        ...
        "strategy": f"Divflip 15m [Paper Bot 5]: {DIV_PIVOT_L}L/{DIV_PIVOT_R}R RSI{RSI_PERIOD} ≤{RSI_LONG_MAX:.0f}/≥{RSI_SHORT_MIN:.0f} / {DCA_LEVELS}DCA @{DCA_SPACING*100:.2f}% / SL {SL_FROM_WORST*100:.1f}% L1 / {tp_str} / BE+{BE_TRIGGER_PCT*100:.1f}% / trail {TRAIL_DIST_PCT*100:.2f}%",
        "paper_mode": True,
