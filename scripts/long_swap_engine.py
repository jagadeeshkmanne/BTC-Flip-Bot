#!/usr/bin/env python3
"""long_swap_engine.py — COPY of short_deepen_engine.run_ec3 with the LONG instrument
swapped from pure-BTC to a constant-weight BTC+ETH basket.

ONLY the instrument the LONG buys changes. Everything else is identical to config D:
  - bull SIGNAL is 100% BTC (4h+daily EMA50/200 AND close>9mo SMA)
  - conviction leverage (long_lev) comes from BTC
  - pyramid@2R / break-even@1R / lock-33%@6R / parabolic de-risk logic identical
  - the SHORT stays exactly config D (ETH, trailing)

LONG basket construction (constant-weight, rebalanced implicitly by normalization):
  normalize BTC and ETH OHLC each to 1.0 at the first COMMON bar:
      btcN = btc / btc.close[0]   (all O/H/L/C divided by btc.close[0])
      ethN = eth / eth.close[0]
  long_basket_close = w_eth*ethN_close + (1-w_eth)*btcN_close, and likewise O/H/L.
  The long uses the basket's OWN ATR(14) for the ATR stop.
  parabolic flag stays from BTC (it's a SIGNAL, derived from BTC SMA).

At w_eth=0.0 the basket == normalized BTC, so the engine MUST reproduce config D
within float noise (only difference is the long instrument is scaled by a constant
btc.close[0], which cancels in returns/units math).
"""
...
def run_swap(w_eth=0.0, short_inst="eth", long_lev=None, lock_frac=0.33, lock_r=6.0,
    ...
    # ---- SIGNALS: all from BTC, identical to config D ----
    ...
    # ---- LONG INSTRUMENT: constant-weight BTC+ETH basket ----
    # normalize each to 1.0 at first common bar (divide all OHLC by that asset's first close)
    bpx0=bc.iloc[0]; epx0=ec.iloc[0]
    btcN={k:btc[k].values/bpx0 for k in ("open","high","low","close")}
    ethN={k:eth[k].values/epx0 for k in ("open","high","low","close")}
    we=w_eth; wb=1.0-w_eth
    # basket OHLC: weighted blend of the two normalized series
    bask_o=we*ethN["open"]+wb*btcN["open"]
    bask_h=we*ethN["high"]+wb*btcN["high"]
    bask_l=we*ethN["low"]+wb*btcN["low"]
    bask_c=we*ethN["close"]+wb*btcN["close"]
    # basket's OWN ATR(14) for the long stop
    bask_df=pd.DataFrame({"open":bask_o,"high":bask_h,"low":bask_l,"close":bask_c})
    bask_atr=bt.atr(bask_df,14).values
    # LONG arrays (basket); SIGNAL arrays still BTC where needed
    lo,lh,ll_,lcv=bask_o,bask_h,bask_l,bask_c; la=bask_atr
    bo,bh,bl,bcv=[btc[k].values for k in ("open","high","low","close")]; ba=bt.atr(btc,14).values

    # ---- SHORT INSTRUMENT: config D = ETH ----
    if short_inst=="eth":
    ...
    def mpx(i): return lcv[i] if side==1 else (scv[i] if side==-1 else lcv[i])
    for i in range(16,n-1):
        if not bull[i]: armedL=True
        if not bear[i]: armedS=True
        covered_this_bar=False
        if side==1:
            lN=ll_[i+1]; oN=lo[i+1]; cN=lcv[i+1]
            if lN<=stop:
        ...
        if side==0:
            ...
            if bull[i] and armedL:
                E=cash; oN=lo[i+1]; st=max(lcv[i]-atr_mult*la[i],lcv[i]*(1-sl_cap)); ent=oN*(1+SLIP)
                if ent-st>0:
