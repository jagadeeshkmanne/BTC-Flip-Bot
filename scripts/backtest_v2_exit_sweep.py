#!/usr/bin/env python3
"""backtest_v2_exit_sweep.py — STOP & EXIT logic sweep on the DEPLOYED btcv2 base.

Copy of run_ec from backtest_v2_eth_conviction with added optional exit logic:
  - chandelier trailing stop (highest-high - tk*ATR) armed after trail_arm_r profit
  - time-based exit (exit long if underwater after tmax_bars bars)
Keeps FEE/SLIP, next-bar fills, shift(1) signals intact. Honest costs, no lookahead.

Base = run_ec_x("eth", long_lev=1.0+1.5*conv) with new params at defaults (off) MUST
reproduce CAGR ~150% / DD -36% / ret/DD 4.18.
"""
...
def run_ec_x(short_inst="btc", long_lev=None, lock_frac=0.33, lock_r=6.0, mf=12, ms=26, msig=9, confirm="macd",
           ...
           # --- NEW exit params (all defaults => identical to base run_ec) ---
           trail_k=0.0, trail_arm_r=0.0, tmax_bars=0, tmax_arm_r=0.0):
    ...
    for i in range(16,n-1):
        ...
        if side==1:
            ...
            bars_in+=1
            hh=max(hh,bh[i])                          # highest high seen on CLOSED bars only
            if lN<=stop:
                ...
                # NOTE: be/pyr/lock above only tighten stop or take partial profit FAVORABLY,
                # so using cN (next-bar close) there is benign. A full time-EXIT decision must
                # NOT peek at cN -> use closed-bar profit at bar i (bcv[i]) for the decision,
                # then fill at next-bar open oN. No lookahead.
                prof_c=(bcv[i]-entry)/R
                # --- chandelier trailing stop (uses CLOSED-bar hh) ---
                if trail_k>0 and prof_c>=trail_arm_r:
                    tstop=hh-trail_k*ba[i]
                    stop=max(stop,tstop)
                # --- time-based exit: flat/underwater after tmax_bars ---
                if tmax_bars>0 and bars_in>=tmax_bars and prof_c<tmax_arm_r:
                    fpx=oN*(1-SLIP); cash+=units*fpx-abs(units)*fpx*FEE; units=0.0; side=0; pyrd=parabd=lockd=False
        elif side==-1:
...
def evalc(**kw):
    s=run_ec_x("eth", long_lev=conv_lev(), **kw)
    cg,dd,rr=m(s)
    o=m(s[s.index>=s.index[int(len(s)*0.6)]])
    years={y:yr(s,y) for y in range(2018,2027)}
    green=all(v>=-1.0 for v in years.values())
    return dict(cg=cg,dd=dd,rr=rr,ocg=o[0],odd=o[1],orr=o[2],green=green,years=years,s=s)


def row(name,r):
    yy="".join(f"{r['years'][y]:+5.0f}" for y in range(2018,2027))
    print(f"{name:<30}{r['cg']*100:>6.0f}%{r['dd']*100:>6.0f}%{r['rr']:>6.2f}{r['orr']:>7.2f}  {'GRN' if r['green'] else 'red'}")
    return yy
