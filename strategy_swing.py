"""
strategy_v8.py — V8: V7 + tuned indicators (1H RSI 21, 1H ATR 20)

V8 changes only 1H indicator periods for better noise filtering:
  - 1H RSI:  14 → 21 (smoother, fewer false momentum readings)
  - 1H ATR:  14 → 20 (wider volatility window, fewer false regime triggers)
  - 4H RSI stays at 14 (confirmation only, not entry filter)

V8 backtest: $5K → $572K (+158.2% CAGR, -20.6% DD, PF 7.29, 62 trades, Calmar 7.68)
vs V7 base:  $5K → $420K (+142.8% CAGR, -20.3% DD, PF 5.54, 73 trades, Calmar 7.03)
→ Slower indicators filter noise: +15pp CAGR, PF 5.54→7.29, same DD, 11 fewer trades.
...
Architecture (3 timeframes):
  ...
  1H    : EXECUTION + entry stack:
            - RSI(21) in pullback zone:   long > 45,  short < 55
            - MACD(12,26,9):              line > signal (long) / < (short)
            - Bullish/Bearish engulfing:  body > 1.0× prev body
            - Volatility filter:          ATR(20) > rolling-50-mean ATR
...
# ─── V7 Pyramiding — scale into winners ───
USE_PYRAMID       = True
...
def run():
    ...
    # 1H indicators
    df["rsi"]        = rsi(df["close"], 21)     # V8: RSI(21) — slower, filters 1H noise
    df["macd_line"], df["macd_signal"] = macd_lines(df["close"])
    df["atr"]        = atr_calc(df, 20)        # V8: ATR(20) — wider volatility window
    ...
    initial_sl_dist_for_pyramid = 0.0
    n_pyramids = 0
    ...
    for i in range(100, len(df)):
        ...
        if (USE_SL_FLIP and pending_flip_side != 0 and position == 0
            ...
            flip_entry_bar = i
            n_flips_taken += 1
            initial_sl_dist_for_pyramid = abs(entry_price - pos_sl)
            pyramided = False
        ...
        def close_and_record(exit_px, reason):
            ...
            nonlocal pyramided, pyramid_entry_price
            ...
            net     = pnl_lev - cost
            # V7: add pyramid position PnL
            pyramid_net = 0.0
            if pyramided and pyramid_entry_price > 0:
                pmp2 = ((exit_px - pyramid_entry_price)/pyramid_entry_price if position == 1
                        else (pyramid_entry_price - exit_px)/pyramid_entry_price)
                pnl2 = pmp2 * LEVERAGE * PYRAMID_FRAC
                cost2 = (TAKER_FEE + SLIPPAGE) * 2 * LEVERAGE * PYRAMID_FRAC
                pyramid_net = pnl2 - cost2
            capital *= (1 + net + pyramid_net)
            # Add back any locked-in partial PnL for the trade-record's pnl_pct display
            full_trade_pnl = net + pyramid_net + (partial_pnls[-1] / 100.0 if (USE_PARTIAL_TP and partial_taken and partial_pnls) else 0.0)
            partial_taken = False
        ...
        # V7 PYRAMIDING — add to winners at +3R
        if USE_PYRAMID and position != 0 and not pyramided and not is_flip:
            if initial_sl_dist_for_pyramid > 0:
                current_r = ((price - entry_price) / initial_sl_dist_for_pyramid if position == 1
                             else (entry_price - price) / initial_sl_dist_for_pyramid)
    ...
    print(f"\n{'='*68}")
    print(f"  STRATEGY V8 (tuned RSI 21 + ATR 20)  |  lev={LEVERAGE}x  |  {fee_label}")
...
<div class="header">
  <h1>Strategy V8 — Tuned Indicators</h1>
  <div class="sub">V7 + RSI(21) + ATR(20) for better noise filtering. Chart: 15m. Exec: 1H. · {summary['fee_label']} · {summary['leverage']:.0f}× lev</div>
</div>
<div class="stats">
